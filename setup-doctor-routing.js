...
export function PublicBookingClientView({
  ...
  async function handleConfirmBooking() {
    ...
    const res = await createRealBookingAction({
      ...
      professionalId: selectedPro?.id, // ENVIA O MÉDICO ESCOLHIDO
    });
    setLoading(false);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between antialiased text-slate-900 font-sans relative">
      ...
      <main className="flex-1 py-10 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-200/80 p-6 sm:p-10">
                ...
                {serviceList.map((svc: any) => (
                  <div 
                    ...
                    onClick={() => {
                      ...
                      // Se tiver mais de 1 médico, vai para a escolha de profissional, senão vai para data
          ...
          {/* PASSO 1.5: ESCOLHA DO MÉDICO / PROFISSIONAL (SE TIVER MAIS DE 1) */}
          {step === 1.5 && (
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">Selecione o médico ou especialista da sua preferência:</p>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <button onClick={handleConfirmBooking} disabled={!clientName || !clientPhone || loading} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-black text-sm rounded-2xl shadow-xl transition-all flex items-center justify-center disabled:opacity-50 cursor-pointer">
                {loading ? "Gravando..." : "Confirmar Agendamento"}
              </button>
            </div>
          )}

          {/* PASSO 4: COMPROVANTE COM MÉDICO CONFIRMADO */}
          {step === 4 && (
            <div className="text-center space-y-6 py-6">
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-left space-y-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-500 font-semibold">Profissional Responsável:</span>
