// fix-production-links.js
const fs = require("fs");
const path = require("path");

console.log("🔗 Tornando os links da página pública 100% dinâmicos para a Vercel...\n");

const files = {
  // 1. COMPONENTE CLIENTE COM DETECÇÃO AUTOMÁTICA DA URL E BOTÃO DE COPIAR
  "src/components/dashboard/share-link-card.tsx": `'use client';
...
export function ShareLinkCard({ slug }: { slug: string }) {
  ...
  // 2. ATUALIZA O DASHBOARD PARA USAR O CARD DINÂMICO
    ...
    <DashboardShell activePage="dashboard">
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1">
              Aqui está o resumo operacional da <span className="text-slate-800 font-bold">{org?.name || 'Viver Bem'}</span> para hoje.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          {/* Card Dinâmico com a URL real da Vercel e botão de Copiar */}
...
console.log("\n🚀 Link da Vercel integrado e dinâmico com botão de copiar!");