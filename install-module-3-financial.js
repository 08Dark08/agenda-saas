// install-module-3-financial.js
const fs = require("fs");
const path = require("path");

console.log("📊 Instalando o Módulo 3: Painel Financeiro Executivo (/financial)...\n");

const files = {
  // ==========================================
  // 1. ATUALIZA A SIDEBAR (DASHBOARD-SHELL) COM A ABA "FINANCEIRO"
  // ==========================================
...
import { 
  ...
  ExternalLink, LogOut, ChevronRight, Settings, DollarSign 
} from 'lucide-react';
...
export function DashboardShell({ 
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={'flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all group ' + (isActive ? 'bg-blue-50/80 text-blue-600 border border-blue-100 shadow-sm' : 'text-slate-600 hover:text-blue-600 hover:bg-slate-50')}
                >
  ...
  // ==========================================
  // 2. PÁGINA COMPLETA DO PAINEL FINANCEIRO (/financial/page.tsx)
  // ==========================================
  "src/app/financial/page.tsx": `import React from 'react';
...
import { 
  ...
  Sparkles, CheckCircle2, Clock, Printer 
} from 'lucide-react';
...
export default async function FinancialPage() {
  ...
  const totalValidAppointments = appointments.filter(a => a.status !== 'CANCELLED');
  const totalVolume = realizedRevenue + projectedRevenue;
  const ticketMedio = totalValidAppointments.length > 0 
    ? Math.round(totalVolume / totalValidAppointments.length) 
    : 0;

  // Agrupamento por Serviço (Ranking de Faturamento)
  const serviceStats: Record<string, { name: string; count: number; totalCents: number }> = {};
  for (const a of totalValidAppointments) {
  ...
  return (
    ...
    >
      <div className="space-y-8">
        ...
        {/* 4 CARDS KPIS FINANCEIROS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {/* Receita Realizada (Concluída) */}
          ...
          {/* Receita Prevista (Confirmadas na agenda) */}
          <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm">
            <div className="flex items-center justify-between mb-3">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium">Serviços que mais geram faturamento para o seu consultório.</p>
            </div>
        ...
        {/* EXTRATO DE LANÇAMENTOS DETALHADO */}
...
fs.writeFileSync(path.join(process.cwd(), files["src/components/layout/dashboard-shell.tsx"]), files["src/components/layout/dashboard-shell.tsx"]); // wait, target paths mapping