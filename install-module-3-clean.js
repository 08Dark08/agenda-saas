...
if (!shellCode.includes("DollarSign")) {
...
if (!shellCode.includes("'financial'")) {
  shellCode = shellCode.replace(
  ...
  console.log("  ✓ Aba 'Financeiro' adicionada com sucesso na Sidebar!");
}

// 2. CRIA A PÁGINA DO PAINEL FINANCEIRO: src/app/financial/page.tsx
const finDir = path.join(process.cwd(), "src/app/financial");
if (!fs.existsSync(finDir)) fs.mkdirSync(finDir, { recursive: true });

const finPageCode = `import React from 'react';
...
fs.writeFileSync(path.join(finDir, "page.tsx"), finPageCode, "utf-8");
console.log("  ✓ Página criada: src/app/financial/page.tsx");

console.log("\n💰 Módulo Financeiro instalado com sucesso!");